...
public class MedicoController {
	...
	public String abrirPorMedico(Medico medico, ModelMap model, @AuthenticationPrincipal User user) {
		if(medico.hasNotId()) {
			medico = service.buscaPorEmail(user.getUsername());
	...
	public String excluirEspecialidePorMedico(@PathVariable("idMed") Long idMed,
						 @PathVariable("idEsp") Long idEsp,
						 RedirectAttributes attr) {
		
		if(service.existeEspecialidadeAgendada(idMed, idEsp)) {
			attr.addFlashAttribute("falha","Existem consultas agendadas é proibida a exclusão.");
		}else {
	...
	// buscar mediaco por especialidade via ajax
