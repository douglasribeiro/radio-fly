...
public class PacienteController {
	...
	public String salvar(Paciente paciente, ModelMap model, @AuthenticationPrincipal User user) {
		Usuario u = usuarioService.buscaPorEmail(user.getUsername());
		if(UsuarioService.isSenhaCorreta(paciente.getUsuario().getSenha(), u.getSenha())) {
			paciente.setUsuario(u);
			service.salvar(paciente);
			model.addAttribute("sucesso", "Seus dados foram inserido com sucesso.");
		} else {
			model.addAttribute("falha", "Sua senha não confere, tente novamante.");
		}
		return "paciente/cadastro";
	}
